...
public class MyCalendarTwo {
    ...
    public boolean book(int start, int end) {
        ...
        for (int[] book : this.books) {
            if (Math.max(book[0], start) < Math.min(book[1], end)) {
                // there is the overlap already
                if (!overlap(Math.max(book[0], start), Math.min(book[1], end), overlaps)) {
                    // overlap in overlaps
                    return false;
                }
            }
    ...
    private boolean overlap(int start, int end, List<int[]> overlaps) {
    ...
    public static void main(String[] args) {
        MyCalendarTwo myObj = new MyCalendarTwo();
        ...
        System.out.println(myObj.book(40,49));
